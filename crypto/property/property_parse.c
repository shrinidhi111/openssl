...
static PS_IDX ossl_property_true, ossl_property_false;
...
static int parse_name(const char *t[], int create, PS_IDX *idx)
{
    char name[100];
    int err = 0;
    size_t i = 0;
 ...
 * The items are sorted for efficient query.  The stack is freed.
 */
static PROPERTY_LIST *stack_to_property_list(STACK_OF(PROPERTY_DEFINITION) *sk)
{
...
PROPERTY_LIST *ossl_parse_property(const char *defn)
{
    PROPERTY_DEFINITION *prop = NULL;
    PROPERTY_LIST *res = NULL;
...
int ossl_property_compare(const PROPERTY_LIST *query, const PROPERTY_LIST *defn)
{
