...
A URI to a store containing trusted CA certificates to use for server certificate verification.
...
B<-CApath>, depending on if the URI indicates a directory or a single
file.
The certificates are also used when building the client certificate chain.
...
Do not load the trusted CA certificates from the default store
