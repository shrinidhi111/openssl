...
sk_TYPE_reserve, sk_TYPE_reserve_at_least, sk_TYPE_free, sk_TYPE_zero,
sk_TYPE_delete, sk_TYPE_delete_ptr, sk_TYPE_push, sk_TYPE_unshift,
sk_TYPE_pop, sk_TYPE_shift, sk_TYPE_pop_free, sk_TYPE_insert, sk_TYPE_set,
sk_TYPE_find, sk_TYPE_find_ex, sk_TYPE_sort, sk_TYPE_is_sorted, sk_TYPE_dup,
sk_TYPE_deep_copy, sk_TYPE_set_cmp_func, sk_TYPE_new_reserve
- stack container
...
=for comment generic
 ...
 int sk_TYPE_reserve_at_least(STACK_OF(TYPE) *sk, int n);
...
sk_TYPE_reserve_at_least() allocates additional memory in the B<sk>
structure such that calls to sk_TYPE_set() will not fail or cause memory to
be allocate or reallocated for positions less than B<n>. If B<n> is less
than the currently allocated size or on error, B<sk> is unchanged. Newly
created elements are set to B<NULL>.
...
sk_TYPE_reserve() and sk_TYPE_reserve_at_least() returns B<1> on successful
allocation of the required memory or B<0> on error.
...
sk_TYPE_reserve_at_least() was added in OpenSSL 3.0.0.
