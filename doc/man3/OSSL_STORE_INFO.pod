...
OSSL_STORE_INFO_clear, OSSL_STORE_INFO_free,
...
=head1 SYNOPSIS
 ...
 void OSSL_STORE_INFO_clear(OSSL_STORE_INFO *store_info);
...
OSSL_STORE_INFO_clear() clears out all B<OSSL_STORE_INFO> data.
This should be used with care, as it may lead to memory leaks.
However, this may be useful if the data pointer has already been
extracted with a C<_get0_> function described here and the data
pointer will be used after freeing the B<OSSL_STORE_INFO>.
