...
#include "../crypto/include/internal/sparse_array.h"
#include "testutil.h"
...
static int test_sparse_array(void)
{
    static const struct {
        ...
        { 22, "a" }, { 0, "z" }, { 1, "b" }, { 290, "c" },
        { UINT_MAX, "m" }, { 6666666, "d" }, { (size_t)-1, "H" },
        { 99, "e" }
    };
    ...
    if (!TEST_ptr(sa = sa_char_new())
            || !TEST_ptr_null(sa_char_get(sa, 3))
            || !TEST_ptr_null(sa_char_get(sa, 0))
            || !TEST_ptr_null(sa_char_get(sa, UINT_MAX)))
        goto err;

    for (i = 0; i < OSSL_NELEM(cases); i++) {
        if (!TEST_true(sa_char_set(sa, cases[i].n, cases[i].v))) {
            TEST_note("iteration %zu", i + 1);
            goto err;
        }
        for (j = 0; j <= i; j++)
            if (!TEST_str_eq(sa_char_get(sa, cases[j].n), cases[j].v)) {
...
static int test_sparse_array_num(void)
{
    ...
    if (!TEST_size_t_eq(sa_char_num(NULL), 0)
            || !TEST_ptr(sa = sa_char_new())
            || !TEST_size_t_eq(sa_char_num(sa), 0))
        goto err;
    for (i = 0; i < OSSL_NELEM(cases); i++)
        if (!TEST_true(sa_char_set(sa, cases[i].n, cases[i].v))
                || !TEST_size_t_eq(sa_char_num(sa), cases[i].num))
            goto err;
    res = 1;
err:
    sa_char_free(sa);
